...
def init_optimizer_state(workload: spec.Workload,
  ...
  optimizer_state = {
      ...
      'ema_weight': hyperparameters.ema_weight,
      'fc_variance_emas': {},
      'reset_threshold': hyperparameters.reset_threshold,
      'init_weight_params': {},
      'init_bias_params': {}
  }
  ...
  def update_fc_stats(fc_layer, args, layer_output):
    with torch.no_grad():
      ...
      first_term = \
        optimizer_state['ema_weight'] * optimizer_state['fc_variance_emas'][fc_layer]
      second_term = \
...
def reset_inactive_params(model, optimizer_state):
  with torch.no_grad():
    fc_variance_emas = optimizer_state['fc_variance_emas']
    reset_threshold = optimizer_state['reset_threshold']

    for submodule in model.modules():
      if submodule not in fc_variance_emas: continue
      reset_mask = (fc_variance_emas[submodule] < reset_threshold)
      ...
      optimizer_state['fc_variance_emas'][submodule][reset_mask] = 1.
